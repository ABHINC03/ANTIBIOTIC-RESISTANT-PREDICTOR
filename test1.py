...
# -----------------------------------------------------------------------------
    ...
    with st.sidebar:
        ...
        if uploaded_file is not None and uploaded_file.name != st.session_state.processed_file_name:
            ...
            if file_ext == 'csv':
                ...
                for col in feature_names:
                    if col in raw_df.columns: temp_df[col] = raw_df[col]
                    ...
                    for i, record in enumerate(records):
                        ...
                        for db_gene, dna_marker in gene_db.items():
                            ...
                            if clean_db_gene in feature_map:
                                if not seq_str or not clean_db_seq: continue
                                
                                # Exact substring match (handles 21bp markers and fragments)
                                if clean_db_seq in seq_str or seq_str in clean_db_seq:
                                    ...
                                    # Threshold against the marker length
            ...
            st.session_state.input_df, st.session_state.sample_ids = temp_df, temp_ids
        ...
        if st.button("↩ Go Back to Menu", use_container_width=True): reset_to_menu()
    ...
    if st.session_state.results_generated and model and not st.session_state.input_df.empty:
        ...
        with col_res2:
            ...
            # --- Gene Display Section (Cleaned up to prevent leak) ---
        ...
        with col_left:
            ...
            if len(detected_genes) > 0:
                plt.rcParams.update({'figure.facecolor': '#1e293b', 'axes.facecolor': '#1e293b', 'text.color': '#e2e8f0', 'axes.labelcolor': '#94a3b8', 'xtick.color': '#94a3b8', 'ytick.color': '#94a3b8', 'grid.color': '#334155'})
            ...
            else: st.info("No markers detected for SHAP mapping.")

        with col_right:
            st.subheader("Resistance Gauge")
            fig_gauge = go.Figure(go.Indicator(mode="gauge+number", value=prob*100, gauge={'bar': {'color': color}, 'bgcolor': "#334155", 'axis': {'range': [0, 100]}}))
